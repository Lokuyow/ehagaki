...
- **flow:** `main.ts` → `applyEmbedSettingsBootstrap()` / `applyUploadDestinationBootstrap()` → iframe の `hostRelayConfig=1` なら `bootstrapIframeHostRelayConfig()` → `App.svelte` → `runAppInitializationBootstrap()`。iframe は別 entrypoint ではなく、この document entry と URL を使う。
