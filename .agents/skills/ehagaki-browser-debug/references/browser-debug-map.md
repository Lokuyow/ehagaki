...
- **Geometryの計測:** host documentのviewport、`ehagaki-composer` host element、Shadow DOMのshell/app/overlay/dialog/footerを別々に`getBoundingClientRect()`とcomputed styleで確認する。Host-owned Lite auto-grow の preferred height は intrinsic `.composer-scroll-content` と KeyboardButtonBar / ReasonInput reservation を測り、Host assigned height、VisualViewport、keyboard inset は含めない。`container-type: inline-size`、component width/height、`--app-overlay-position`、DialogWrapperのcontainer-layout absolute positioningを確認し、Shadow DOMだけでfixed要素がhost-relativeになると仮定しない。
