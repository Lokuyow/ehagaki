...
iframe が `ready` を送った後は、親ページから `composer.setContext` を送ることで、iframe を再読み込みせずに reply / quote や本文を差し替えられます。
...
- `composer.setContext` は patch として扱われます。`undefined` は変更なし、`reply: null` は reply 解除、`quotes: []` または `quotes: null` は quote 全解除、`content: null` は本文クリアです
