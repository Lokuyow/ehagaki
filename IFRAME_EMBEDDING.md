...
このサンプルは親ページの `window.nostr` を使って `signEvent` / `nip04.*` / `nip44.*` を委譲し、受信時には namespace、version、type、requestId、payload shape を検証します。`ready` を受け取っただけでは自動ログインせず、`auth.login` ボタンを押したときだけ iframe にログイン同期を通知します。
...
2. 親が iframe をログイン状態へ同期したいタイミングで `auth.login` を送る
