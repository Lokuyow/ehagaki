...
<iframe
  src="https://lokuyow.github.io/ehagaki/?channel=nevent1...&channelName=General&channelAbout=Public%20chat&channelPicture=https%3A%2F%2Fexample.com%2Fchannel.png"
  allow="local-network-access; local-network; loopback-network"
  width="600"
  height="400">
</iframe>
...
- `channelRelay` は任意で複数指定できます。`channel` が `note1...` の場合の relay hint 補助に使えます
- `channelName`、`channelAbout`、`channelPicture` は任意です。親が渡した場合は iframe 側でそのまま preview に使います
- `channelName`、`channelAbout`、`channelPicture` が 1 つも渡されなかった場合は、iframe 側が relay に REQ して kind 40 / kind 41 event から metadata と relay 候補を取得します
...
- `composer.setContext` の `channel` は `{ reference, relayHints?, name?, about?, picture? }` です。`reference` は `note1...` / `nevent1...` を使います
- `channel.name` / `channel.about` / `channel.picture` のうち 1 つでも渡された場合は、その値を iframe 側の channel preview に使います
- `channel.name` / `channel.about` / `channel.picture` が 1 つも渡されなかった場合は、iframe 側が relay に REQ して kind 40 / kind 41 から `name` / `about` / `picture` / relay 候補を解決します
...
- `composer.contextUpdated` の payload は `{ timestamp, reply, quotes, channel }` です。`channel` は `{ reference, relayHints?, name?, about?, picture? } | null` で返ります。`reply` は未設定時に `null`、`quotes` は常に配列です
