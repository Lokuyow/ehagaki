<script lang="ts">
    ...
    import { ehagakiDb, type PostHistoryRecord } from '../../lib/storage/ehagakiDb';
    ...
    const posts = Array.from({ length: TOTAL_POSTS }, (_, index) => buildPost(index));
    ...
    onMount(async () => {
        ...
        await ehagakiDb.postHistory.where('pubkeyHex').equals(HARNESS_PUBKEY).delete();
...
<style>
    :global(body) {
        ...
        margin: 0;
        background:
            radial-gradient(circle at top left, rgba(27, 85, 138, 0.14), transparent 32%),
