...
describe('LoginDialog', () => {
    ...
    it('待受準備中は QR / copy / direct-open を利用可能にしない', async () => {
        ...
        expect(screen.getAllByText('接続コードを準備しています...').length).toBeGreaterThanOrEqual(1);
    ...
    it('direct-open 後も同じ pending 接続でページが表示中なら案内 toast を表示する', async () => {
        vi.useFakeTimers();
        const assign = vi.fn();
        vi.stubGlobal('location', { assign });
    ...
    it('direct-open 案内は pending 終了と URI 再生成で cleanup する', async () => {
        vi.useFakeTimers();
        const assign = vi.fn();
        vi.stubGlobal('location', { assign });
        const uri = 'nostrconnect://client?relay=wss%3A%2F%2Frelay.example.com%2F&name=eHagaki';
        const nextUri = 'nostrconnect://client?relay=wss%3A%2F%2Frelay.next.example.com%2F&name=eHagaki';
    ...
    it('接続コードが最新のときは再生成ボタンを無効化し、relay 編集で再び有効化する', async () => {
        const onNostrConnectStart = vi
            .fn<(relays: string[]) => Promise<string | undefined>>()
            .mockResolvedValue(undefined);
        const uri =
            'nostrconnect://client?relay=wss%3A%2F%2Fnostr.oxtr.dev%2F&relay=wss%3A%2F%2Ftheforest.nostr1.com%2F&relay=wss%3A%2F%2Frelay.primal.net%2F&relay=wss%3A%2F%2Fephemeral.snowflare.cc%2F&name=eHagaki';
        ...
        expect(
            (screen.getByTestId('nostrconnect-regenerate') as HTMLButtonElement)
                .disabled,
        ).toBe(true);

        const relayInputs = screen.getAllByPlaceholderText(
