...
### 1. editor-history.integration.test.ts (10テスト) ⭐ NEW

エディターの履歴管理機能をテストします。ペースト操作の履歴の独立性と、Undo/Redoの正確性を検証します。
...
- **基本的な履歴操作** (2テスト)
  - テキスト入力→Undoで元に戻ること
  - 複数回のテキスト入力→複数回のUndoで順に元に戻ること

- **ペースト操作の履歴管理** (2テスト)
  - ペースト操作が独立した履歴エントリとして記録されること
  - テキスト入力→ペースト→Undoでペーストのみ消えること

- **高速入力時のペースト操作（Issue修正の検証）** (3テスト)
  - ペースト操作のメタデータが正しく設定されること
  - ペースト直後の入力にもrebased=0が設定されること
  - 300ms以内の連続入力は同じグループにまとめられること

- **複雑なシナリオ** (1テスト)
  - 連続したペースト操作のメタデータが正しく設定されること
...
**関連ファイル:**
- `src/lib/editor/customHistoryPlugin.ts`
- `src/lib/editor/clipboardExtension.ts`
- `src/lib/editor/contentTracking.ts`

### 2. auth-to-post.integration.test.ts (18テスト)
