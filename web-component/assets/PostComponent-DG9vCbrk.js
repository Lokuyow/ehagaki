...
function Rt(n, t) {
  ...
  let o = z(t, "item", 7), l = z(t, "index", 7), d = z(t, "onDelete", 7), f = z(t, "onDragStart", 7), w = z(t, "onDragOver", 7), F = z(t, "onDragEnd", 7), I = z(t, "onDrop", 7), p = z(t, "onTouchDragStart", 7), _ = z(t, "disabled", 7, !1), R = Q(void 0), M = Q(void 0);
  const J = tr();
  Zn(() => e(R), {
    onLongPress: (g, A) => {
      _() || p()?.(l(), g, A);
  ...
  }), he("dragstart", m, re), he("dragover", m, Se), he("drop", m, ce), he("dragend", m, () => F()()), we("click", P, function(...g) {
...
function Ht(n, t) {
  ...
  function le(v, h) {
    ...
    if (D) {
      ...
      S(l, h.clientX < T.left + T.width / 2 ? v : v + 1, !0);
    } else
      S(l, v, !0);
  }
  function Y() {
  ...
  function O(v) {
    ...
    if (h && h.length > 0) {
      ...
      v.clientX < j.left ? S(l, 0) : v.clientX > D.right && S(l, e(R).length, !0);
    }
    if (e(p)) {
  ...
  function re(v, h) {
    ...
    const j = e(p).getBoundingClientRect(), D = v === "left" ? h - j.left : j.right - h, T = Math.max(0, Math.min(1, D / ke)), q = St + (rr - St) * (1 - T), L = () => {
      if (!e(p)) return;
  ...
  function Se(v, h, j) {
    ...
    if (D) {
      ...
      F = T.width * L / 2, I = T.height * L / 2, w = D.cloneNode(!0), w.style.cssText = `
                ...
                z-index: 9999;
                border-radius: 6px;
            `, zr().overlayTarget.appendChild(w);
    }
  ...
  var H = Nr(), P = At(H);
  {
    var ve = (v) => {
      ...
      nr(h, 23, () => e(R), (D) => D.id, (D, T, q) => {
        ...
        }), pe(L), be(() => V = Fe(L, 1, "gallery-item-wrapper svelte-w2vv8k", null, V, {
          "insert-bar-left": e(J) === e(q),
          "insert-bar-right": e(J) === e(R).length && e(q) === e(R).length - 1
        })), ne(D, L);
      }), pe(h), Ce(h, (D) => S(p, D), () => e(p)), be(
        (D) => {
...
function ri(n) {
  ...
  function r(o) {
    ...
    Ot(l) && !d ? (o.preventDefault(), e(t) || (S(t, !0), n.classList.add("drag-over"))) : e(t) && (S(t, !1), n.classList.remove("drag-over"));
  }
  function a(o) {
    e(t) && (S(t, !1), n.classList.remove("drag-over"));
  }
  async function s(o) {
    if (S(t, !1), n.classList.remove("drag-over"), De(n) || !Te(n)) {
      o.preventDefault();
      return;
    }
...
function ai(n) {
  function t(a) {
    ...
    if (s && (s.closest('.editor-image-button[data-dragging="true"]') || s.closest('.custom-emoji-drag-target[data-dragging="true"]'))) {
      const o = a.touches[0], l = 120, d = bt.querySelector(".tiptap-editor");
      if (d) {
  ...
  function r(a) {
    const s = bt.querySelectorAll(".drop-zone-indicator");
    s.forEach((o) => {
...
const Li = {
  ...
  .editor-account-placeholder-fallback {display:block;width:100%;height:100%;border-radius:50%;}.editor-account-placeholder-image {object-fit:cover;}.editor-container.account-avatar-placeholder.svelte-15ticnd
    p.is-editor-empty:first-child::before {padding-left:38px;}.editor-container.sending.svelte-15ticnd {background:color-mix(in srgb, var(--surface-editor) 82%, var(--surface-button) 18%);cursor:not-allowed;}.editor-container.sending.svelte-15ticnd .tiptap-editor {cursor:not-allowed;opacity:0.72;}.editor-container.editor-submit-enabled.sending.svelte-15ticnd .tiptap-editor {pointer-events:none;}.editor-container.sending.svelte-15ticnd .editor-image-button,
  .editor-container.sending.svelte-15ticnd .custom-emoji-drag-target,
  .editor-container.sending.svelte-15ticnd .media-delete-btn {pointer-events:none;}.editor-container.editor-submit-enabled.svelte-15ticnd .tiptap-editor {padding-inline-end:calc(var(--post-editor-block-padding) + 48px);}.editor-submit-button-container.svelte-15ticnd {position:absolute;inset-inline-end:var(--post-editor-block-padding);bottom:var(--post-editor-block-padding);z-index:4;}.editor-submit-button {width:40px;height:40px;flex:0 0 40px;}button.editor-submit-button .plane-icon.svg-icon {width:22px;height:22px;mask-image:var(--ehagaki-icon-70617065722d706c616e652d736f6c69642d66756c6c2e737667);margin-inline-end:1px;margin-top:1px;}.editor-container.drag-over.svelte-15ticnd {border:3px dashed var(--theme);}
...
function Ai(n, t) {
  Ze(t, !0), Ge(n, Li);
  const r = () => $e(Ye, "$_", a), [a, s] = Qe();
  let o = z(t, "rxNostr", 7), l = z(t, "hasStoredKey", 7), d = z(t, "hasPostingCapability", 23, l), f = z(t, "isSwitchingAccount", 7, !1), w = z(t, "onPostSuccess", 7), F = z(t, "availableComposerHeight", 7, Pe), I = z(t, "minEditorHeight", 7, Pe), p = z(t, "onCustomEmojiSelect", 7), _ = z(t, "onEditorEmptyChange", 7), R = z(t, "notificationPort", 7), M = z(t, "hostOwnedConfig", 7), J = z(t, "hostCustomEmojiItems", 23, () => []), ee = z(t, "normalUploadFiles", 7);
  const Y = !1;
  let X = E(() => !Y), O = E(() => $.isUploading), de = E(() => $.canPost), G = E(() => Y), re = Q(null), c = Q(null), Se = Q(!1), ce = Q(void 0), x = Q(void 0), k = Q(Oe({})), m = Q(Oe({})), H = E(() => Ue.value), P = E(() => $.postStatus), ve = E(() => $.uploadErrorMessage), v = E(() => Or.value), h = E(() => _r.value), j = E(() => kr.value), D = Q(!0), T = !1, q = E(() => l() && !f() && e(h) && !e(j) && e(D)), L = null, V = null, g = null, A = null, U = Q(Oe(Pe)), K = E(() => Y), _e = E(() => e(K) ? `--post-editor-auto-grow-min-lines: ${M().editorMinLines}lh; --post-editor-auto-grow-max-lines: ${M().editorMaxLines}lh;` : `--post-editor-min-height: ${I()}px; --post-editor-target-height: ${e(U)}px;`), et = E(() => r()("postComponent.enter_your_text") || "テキストを入力してください");
  ye(() => {
    e(c), pr(e(et));
  }), ye(() => {
  ...
  const Ne = Kr({
    ...
    uploadFiles: async (i) => {
        ...
        return null;
      {
        ...
        const { uploadFiles: u } = await import("./App-94Lvneu9.js").then((b) => b.eB);
        return await u(i);
      }
    }
  }), Ee = pi({
  ...
  ye(() => {
    ...
    g = xi({
      ...
      eventCallbacks: {
        onContentUpdate: Ir,
        onImageFullscreenRequest: (y, N, ie) => {
          ue.showImageFullscreen(y, N, ie || "");
        },
        onSelectImageNode: (y) => {
        }
      }
    ...
    let i = null;
    const u = (y) => {
      const N = y.isEmpty, ie = !T || e(D) !== N;
      S(D, N, !0), T = !0, ie && _()?.(N);
    }, b = ({ editor: y }) => {
      u(y);
    };
    A = e(re).subscribe((y) => {
      i && i.off("transaction", b), i = y, S(c, y, !0), y && u(y), y?.on("transaction", b), We.set(y);
    });
    const C = (y) => {
      const N = y, { src: ie, alt: je, mediaId: In } = N.detail;
      ue.showImageFullscreen(ie, je, In || "");
    };
    return window.addEventListener("image-fullscreen-request", C), () => {
      window.removeEventListener("image-fullscreen-request", C), g && (i && i.off("transaction", b), Pi({
  ...
  const Yt = Ne.handleFileSelect;
  async function Vt(i) {
    return await Ne.performUpload(i);
  }
  function Zt(i) {
    if (!e(c) || !i) return;
    const u = e(
...
`).map((C) => ({
    ...
    return e(c).isEmpty ? e(c).commands.setContent({ type: "doc", content: b }) : e(c).chain().focus("end").insertContent([{ type: "paragraph" }, ...b]).run(), e(c).commands.focus("end"), !0;
  }
  function en(i) {
    if (!e(c) || !i) return;
    const u = Sr(i);
  ...
  function nn(i) {
    ...
    let b = e(c).state.tr, C = e(c).state.doc.content.size;
    i.forEach((y) => {
      ...
      if (y.type === "image" && u.nodes.image) {
        ...
        b = b.insert(C, ie), C += ie.nodeSize, y.ox && S(k, { ...e(k), [N]: y.ox }, !0), y.x && S(m, { ...e(m), [N]: y.x }, !0);
      } else if (y.type === "video" && u.nodes.video) {
        const ie = u.nodes.video.create({ src: N });
        b = b.insert(C, ie), C += ie.nodeSize;
      }
    }), e(c).view.dispatch(b), e(c).commands.focus("end");
  }
  function rn(i) {
  ...
  function ot(i) {
    ...
    ze();
    const { state: u, view: b } = e(c), C = i < 0 ? u.selection.from : u.selection.to, y = Math.max(0, Math.min(u.doc.content.size, C + i));
    if (y === C) return;
    const N = xr.near(u.doc.resolve(y), i);
    b.dispatch(u.tr.setSelection(N).scrollIntoView().setMeta("addToHistory", !1));
  }
  function on() {
    ot(-1);
  }
  ...
  function sn() {
    if (!e(c) || e(P).sending) return;
    ze();
    const { state: i, view: u } = e(c), { selection: b } = i;
    if (!b.empty) {
      e(c).commands.deleteSelection();
      return;
    }
    const y = b.$from.nodeBefore;
    if (y) {
      const N = y.isText ? Array.from(y.text ?? "").at(-1)?.length ?? 0 : y.nodeSize;
      N > 0 && u.dispatch(i.tr.delete(b.from - N, b.from).scrollIntoView());
      return;
    }
    e(c).commands.first(({ commands: N }) => [
      () => N.joinBackward(),
      () => N.selectNodeBackward()
    ]);
  }
  function ln() {
    !e(c) || e(P).sending || (ze(), e(c).commands.keyboardShortcut("Enter"));
  }
  function at() {
  ...
  function st() {
    ...
    if (e(c)) {
      ...
      e(c).chain().clearContent().run(), Ft.reset(), _t.reset(), ae.clearAll(), S(k, {}, !0), S(m, {}, !0), Ke(), i.length > 0 && e(c).commands.insertContent(` ${i.map((u) => `#${u}`).join(" ")}`), e(c).commands.focus("start");
    }
  }
  async function cn() {
    if (!at()) return;
  ...
  ye(() => {
    const i = ae.items.some((C) => !C.isPlaceholder), u = !!$.content.trim(), b = $.hasImage;
    $.canPost = u || b || i;
  });
  let lt = !0;
  ye(() => {
    ...
    if (i)
      ...
      const b = Ie(() => ae.getItems()), C = bi({ currentEditor: u, items: b });
      C.hadItems && Ie(() => {
        S(k, C.imageOxMap, !0), S(m, C.imageXMap, !0);
      }), Ie(() => ae.clearAll());
    }
  });
  var vn = {
  ...
  pe(Z), Re(Z, (i, u) => ii?.(i, u), () => ({ dragOver: (i) => S(Se, i, !0) })), Re(Z, (i) => oi?.(i)), Re(Z, (i) => ai?.(i)), Re(Z, (i) => si?.(i)), Ce(Z, (i) => V = i, () => V);
  var pt = oe(Z, 2);
  {
    var bn = (i) => {
      Ht(i, {});
  ...
  var ft = oe(pt, 2);
  {
    var En = (i) => {
      var u = Ii();
      Ce(u, (b) => S(ce, b), () => e(ce)), we("change", u, Yt), ne(i, u);
    };
    te(ft, (i) => {
      e(X) && i(En);
    });
  ...
  var vt = oe(xe, 2);
  {
    var Cn = (i) => {
      {
        let u = E(() => r()("postComponent.warning")), b = E(() => r()("postComponent.secret_key_detected")), C = E(() => r()("postComponent.post")), y = E(() => r()("postComponent.cancel"));
        Rr(i, {
          get open() {
            return e(Ut);
          },
  ...
  ), we("click", Z, Wt), he("keydown", Z, zt, !0), we("keydown", Z, Nt), we("beforeinput", Z, Bt), ne(n, dt);
  var kn = Je(vn);
  return s(), kn;
}
